        ...
        static void RebarTest()
        {
            ...
            foreach (var mark in marks)
            {
                ...
                foreach (LineSegment segment in segments)
                {
                    ...
                    blockLines.Add(new GeoLine(segment.StartPoint.ToGeoPoint(), segment.EndPoint.ToGeoPoint()));
                }
            ...
            foreach (var dimension in dimensions)
            {
                if (dimension is StraightDimensionSet straightDimensionSet)
                {
                    ...
                    GeoPoint[] vertex =
                    [
                        new GeoPoint(dimBox.MinPoint.X, dimBox.MinPoint.Y),
                        new GeoPoint(dimBox.MinPoint.X, dimBox.MaxPoint.Y),
                        new GeoPoint(dimBox.MaxPoint.X, dimBox.MaxPoint.Y),
                        new GeoPoint(dimBox.MaxPoint.X, dimBox.MinPoint.Y),
                    ];
                    // Add the bounding polygon to the list of blocked regions
                    blockPolygons.Add(new GeoPolygon(vertex));
                }
            }
