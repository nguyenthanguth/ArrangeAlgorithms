...
namespace ArrangeAlgorithms.CadTest
{
    ...
    {
        /// <summary>
        /// Chạy test JOIN bằng <see cref="Merge.Join"/>, bản dùng lưới không gian.
        /// </summary>
        public void RunJoinTest()
        {
            Run("Merge.Join", Merge.Join, "Joined_Lines", 2);
        }

        /// <summary>
        /// Chạy test JOIN bằng <see cref="Merge.JoinBackup"/>, bản so từng cặp giữ lại để đối chiếu.
        /// </summary>
        /// <remarks>
        ...
        {
            Run("Merge.JoinBackup", Merge.JoinBackup, "Joined_Lines_Backup", 4);
        }
        ...
        private void Run(
            string label,
            Func<IEnumerable<GeoLine>, Tolerance, GeoPolyline[]> join,
            string layerName,
            short colorIndex)
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    GeoPolyline[] joinedPolylines = join(geoLines, Tolerance.Global);
                    watch.Stop();
                    ...
                    foreach (GeoPolyline geoPolyline in joinedPolylines)
                    {
                    ...
                    foreach (GeoLine geoLine in geoLines)
                    {
