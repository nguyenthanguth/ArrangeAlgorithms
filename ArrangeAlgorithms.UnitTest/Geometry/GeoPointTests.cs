...
namespace ArrangeAlgorithms.UnitTest.Geometry
{
    public class GeoPointTests
    {
        ...
        {
            var ptInside = new GeoPoint(5, 5);
            var ptOnSide = new GeoPoint(10, 5);
            var ptOutSide = new GeoPoint(15, 5);

            var rect = new GeoRectangle(new GeoPoint(5, 5), 10, 10, 0);
        ...
        public void Point_ProjectTo_WorksCorrectlyForShapes()
        {
            ...
            Assert.True(projLine.IsEqualTo(new GeoPoint(5, 0)));

            var projCircle = new GeoPoint(10, 0).GetClosestPointOnBoundary(circle);
            Assert.True(projCircle.IsEqualTo(new GeoPoint(5, 0)));
        }

        [Fact]
        public void Point_DistanceTo_WorksCorrectlyForShapes()
        {
            var pt = new GeoPoint(0, 10);
            var circle = new GeoCircle(new GeoPoint(0, 0), 4);
            var rect = new GeoRectangle(new GeoPoint(0, 0), 10, 6, 0);
