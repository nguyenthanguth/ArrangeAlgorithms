...
namespace ArrangeAlgorithms.UnitTest.Core
{
    public class ParametrizationTests
    {
        ...
        private static GeoCircle Circle() => new GeoCircle(new GeoPoint(0, 0), 10);

        private static GeoRectangle Rect() => new GeoRectangle(new GeoPoint(0, 0), 20, 10);
        ...
        {
            Assert.True(Segment().GetPointAtParameter(parameter).IsEqualTo(new GeoPoint(x, y)));
        }
        ...
        public void Line_ParameterAndDistance_AreProportional()
        {
            ...
            Assert.True(line.GetPointAtDistance(7.5).IsEqualTo(new GeoPoint(7.5, 0)));
            Assert.Equal(7.5, line.GetDistanceAtPoint(new GeoPoint(7.5, 4)), 9);
            Assert.Equal(0.75, line.GetParameterAtPoint(new GeoPoint(7.5, 4)), 9);
        }
        ...
        public void Line_ExtrapolatesBeyondItsEndpoints()
        {
            ...
            Assert.True(line.GetPointAtParameter(-0.5).IsEqualTo(new GeoPoint(-5, 0)));
            Assert.True(line.GetPointAtParameter(2.0).IsEqualTo(new GeoPoint(20, 0)));
            Assert.True(line.GetPointAtDistance(-5.0).IsEqualTo(new GeoPoint(-5, 0)));

            Assert.Equal(-0.5, line.GetParameterAtPoint(new GeoPoint(-5, 3)), 9);
            Assert.Equal(-5.0, line.GetDistanceAtPoint(new GeoPoint(-5, 3)), 9);
        }

        [Fact]
        public void Line_DegenerateSegment_ReportsZeroParameter()
        {
            var degenerate = new GeoLine(new GeoPoint(4, 4), new GeoPoint(4, 4));

            Assert.Equal(0.0, degenerate.GetParameterAtDistance(10.0), 9);
            Assert.Equal(0.0, degenerate.GetDistanceAtParameter(0.5), 9);
            Assert.True(degenerate.GetPointAtDistance(3.0).IsEqualTo(new GeoPoint(4, 4)));
        }
        ...
        {
            Assert.True(Circle().GetPointAtParameter(parameter).IsEqualTo(new GeoPoint(x, y)));
        }
        ...
        public void Circle_DistanceIsArcLength()
        {
            ...
            Assert.True(circle.GetPointAtDistance(circumference * 0.25).IsEqualTo(new GeoPoint(0, 10)));
        }
        ...
        public void Circle_ParameterAtPoint_UsesOnlyTheDirectionFromTheCentre()
        {
            ...
            Assert.Equal(0.25, circle.GetParameterAtPoint(new GeoPoint(0, 10)), 9);
            Assert.Equal(0.25, circle.GetParameterAtPoint(new GeoPoint(0, 3)), 9);
            Assert.Equal(0.0, circle.GetParameterAtPoint(new GeoPoint(50, 0)), 9);
        }

        [Fact]
        public void Circle_ZeroRadius_ReportsZeroParameter()
        {
            var point = new GeoCircle(new GeoPoint(1, 2), 0.0);

            Assert.Equal(0.0, point.GetParameterAtDistance(5.0), 9);
            Assert.True(point.GetPointAtDistance(5.0).IsEqualTo(new GeoPoint(1, 2)));
        }
        ...
        public void Polygon_WalksThePerimeterFromTheFirstVertex()
        {
            ...
            Assert.True(poly.GetPointAtParameter(0.0).IsEqualTo(new GeoPoint(0, 0)));
            Assert.True(poly.GetPointAtDistance(5.0).IsEqualTo(new GeoPoint(5, 0)));
            Assert.True(poly.GetPointAtDistance(10.0).IsEqualTo(new GeoPoint(10, 0)));
            Assert.True(poly.GetPointAtDistance(15.0).IsEqualTo(new GeoPoint(10, 5)));
            Assert.True(poly.GetPointAtParameter(0.25).IsEqualTo(new GeoPoint(10, 0)));
        }
        ...
        public void Polygon_DistanceAtPoint_MeasuresAlongTheBoundary()
        {
            ...
            Assert.Equal(5.0, poly.GetDistanceAtPoint(new GeoPoint(5, 0)), 9);
            Assert.Equal(15.0, poly.GetDistanceAtPoint(new GeoPoint(10, 5)), 9);

            // A point off the boundary is measured at its closest point on the boundary.
            Assert.Equal(5.0, poly.GetDistanceAtPoint(new GeoPoint(5, -3)), 9);
            Assert.Equal(0.125, poly.GetParameterAtPoint(new GeoPoint(5, -3)), 9);
        }
        ...
        public void Polyline_Open_ClampsOutsideItsEnds()
        {
            ...
            Assert.True(path.GetPointAtParameter(0.0).IsEqualTo(new GeoPoint(0, 0)));
            Assert.True(path.GetPointAtParameter(0.5).IsEqualTo(new GeoPoint(10, 0)));
            Assert.True(path.GetPointAtParameter(1.0).IsEqualTo(new GeoPoint(10, 10)));

            // An open path has no natural extension, so it stops at its endpoints.
            Assert.True(path.GetPointAtParameter(-1.0).IsEqualTo(new GeoPoint(0, 0)));
            Assert.True(path.GetPointAtParameter(2.0).IsEqualTo(new GeoPoint(10, 10)));
            Assert.True(path.GetPointAtDistance(-5.0).IsEqualTo(new GeoPoint(0, 0)));
            Assert.True(path.GetPointAtDistance(100.0).IsEqualTo(new GeoPoint(10, 10)));
        }
        ...
        public void Polyline_DistanceAndParameter_AreProportional()
        {
            ...
            Assert.Equal(15.0, path.GetDistanceAtPoint(new GeoPoint(10, 5)), 9);
            Assert.Equal(0.75, path.GetParameterAtPoint(new GeoPoint(10, 5)), 9);
        }
        ...
        public void ResultOfPointAtParameter_LiesOnTheCurve()
        {
            ...
            {
                Assert.True(Containment.IsPointOn(poly, poly.GetPointAtParameter(parameter)));
                Assert.True(Containment.IsPointOn(loop, loop.GetPointAtParameter(parameter)));
                Assert.True(Containment.IsPointOn(circle, circle.GetPointAtParameter(parameter)));

                GeoPoint onRect = rect.GetPointAtParameter(parameter);
                bool onAnEdge = false;
                foreach (GeoLine edge in rect.GetEdges())
                {
                    if (Containment.IsPointOn(edge, onRect)) onAnEdge = true;
                }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => Parametrization.GetPointAtParameter((GeoPolyline)null, 0.5));
            Assert.Throws<ArgumentNullException>(() => Parametrization.GetDistanceAtPoint((GeoPolygon)null, new GeoPoint(0, 0)));
        }
    }
}
