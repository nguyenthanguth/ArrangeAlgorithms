...
{
    /// <summary>
    /// Covers the parts of <see cref="Merge"/> that guard against bad input, and the parts of
    /// <see cref="Merge.Join"/> that decide how a run is grown rather than what it looks like once grown.
    /// </summary>
    public class MergeJoinTests
    {
        ...
        {
            var ex = Assert.Throws<ArgumentNullException>(() => Merge.ConsecutiveLines(null, Tolerance.Global));
        ...
        {
            var ex = Assert.Throws<ArgumentNullException>(() => Merge.ConsecutivePolylines(null, Tolerance.Global));
        ...
        {
            var second = new GeoPolyline(new GeoPoint(0, 0), new GeoPoint(1, 0));
            var ex = Assert.Throws<ArgumentNullException>(() => Merge.Polylines(null, second, Tolerance.Global));
        ...
        {
            var first = new GeoPolyline(new GeoPoint(0, 0), new GeoPoint(1, 0));
            var ex = Assert.Throws<ArgumentNullException>(() => Merge.Polylines(first, null, Tolerance.Global));
        ...
        {
            var ex = Assert.Throws<ArgumentNullException>(() => Merge.Join(null, Tolerance.Global));
        ...
        {
            // The public GeoPolyline constructor rejects two coincident vertices, so Join must not be
            ...
            var result = Merge.Join(segments, Tolerance.Global);
        ...
        public void Join_AllSegmentsZeroLength_ReturnsNothing()
        {
            var segments = new[]
            {
                ...
                new GeoLine(new GeoPoint(9, 1), new GeoPoint(9, 1))
            };
        ...
        public void Join_ZeroLengthSegmentAmongRealOnes_IsIgnored()
        {
            ...
            AssertVertices(result[0], new GeoPoint(0, 0), new GeoPoint(5, 0), new GeoPoint(5, 5));
        }
        ...
        public void Join_SegmentShorterThanTolerance_CountsAsZeroLength()
        {
            ...
            var segments = new[] { new GeoLine(new GeoPoint(0, 0), new GeoPoint(0.1, 0)) };

            Assert.Empty(Merge.Join(segments, tolerance));

            // The same segment survives once the tolerance no longer swallows it.
            Assert.Single(Merge.Join(segments, new Tolerance(0.01, 0.01)));
        }

        [Fact]
        public void Join_EmptyInput_ReturnsNothing()
        {
            Assert.Empty(Merge.Join(new List<GeoLine>(), Tolerance.Global));
        }
        ...
        public void Join_EveryResultHasDistinctNeighbouringVertices()
        {
            ...
            var awkward = new List<GeoLine[]>
            {
                ...
                new[]
                {
                    ...
                    new GeoLine(new GeoPoint(10, 0), new GeoPoint(4, 0)),    // folds back
                ...
                new[]
                {
                    ...
                    new GeoLine(new GeoPoint(4, 0), new GeoPoint(4, 4)),
                    new GeoLine(new GeoPoint(4, 4), new GeoPoint(0, 0)),     // closes the loop
        ...
        public void Join_SeedInTheMiddle_GrowsBothWays()
        {
            ...
            var segments = new[]
            {
                new GeoLine(new GeoPoint(5, 0), new GeoPoint(5, 5)),
                new GeoLine(new GeoPoint(0, 0), new GeoPoint(5, 0)),
                new GeoLine(new GeoPoint(5, 5), new GeoPoint(10, 5))
            };

            var result = Merge.Join(segments, Tolerance.Global);

            Assert.Single(result);
            AssertVertices(result[0], new GeoPoint(0, 0), new GeoPoint(5, 0), new GeoPoint(5, 5), new GeoPoint(10, 5));
        }
        ...
        public void Join_ForkedInput_GivesTheSameAnswerEveryTime()
        {
            ...
            var first = Describe(Merge.Join(segments, Tolerance.Global));
            for (int attempt = 0; attempt < 5; attempt++)
            {
                Assert.Equal(first, Describe(Merge.Join(segments, Tolerance.Global)));
            }

            // Both branches survive; the fork splits the input into two runs rather than losing one.
            Assert.Equal(2, Merge.Join(segments, Tolerance.Global).Length);
        }
        ...
        public void Join_GapWithinTolerance_IsBridged()
        {
            ...
            {
                new GeoLine(new GeoPoint(0, 0), new GeoPoint(5, 0)),
                new GeoLine(new GeoPoint(5.5, 0), new GeoPoint(5.5, 5))   // 0.5 away from the first end
            };

            Assert.Single(Merge.Join(segments, new Tolerance(1.0, 1.0)));
            Assert.Equal(2, Merge.Join(segments, new Tolerance(0.1, 0.1)).Length);
        }
        ...
        public void Join_ZeroTolerance_MatchesExactEndpointsOnly()
        {
            ...
            Assert.Single(Merge.Join(exact, new Tolerance(0.0, 0.0)));
            Assert.Equal(2, Merge.Join(nearMiss, new Tolerance(0.0, 0.0)).Length);
        }
        ...
        public void Join_ZeroTolerance_JoinsRunsMeetingAtTheOrigin()
        {
            ...
            {
                new GeoLine(new GeoPoint(-5, 0), new GeoPoint(0, 0)),
                new GeoLine(new GeoPoint(0, 0), new GeoPoint(0, 5))
            };
            var awayFromOrigin = new[]
            {
                new GeoLine(new GeoPoint(7, 3), new GeoPoint(12, 3)),
                new GeoLine(new GeoPoint(12, 3), new GeoPoint(12, 8))
            };

            Assert.Single(Merge.Join(throughOrigin, zero));
            Assert.Single(Merge.Join(awayFromOrigin, zero));

            // Together they are two runs, not one: nothing has been swept into a shared bucket and
            // joined by accident.
            var both = new List<GeoLine>(throughOrigin);
            both.AddRange(awayFromOrigin);
            Assert.Equal(2, Merge.Join(both, zero).Length);
        }
        ...
        public void Join_ZeroTolerance_StillSpreadsSegmentsOverTheGrid()
        {
            ...
            var zero = new Tolerance(0.0, 0.0);
            var segments = new List<GeoLine>(count);
            for (int i = 0; i < count; i++)
            {
                segments.Add(new GeoLine(new GeoPoint(i, i % 2), new GeoPoint(i + 1, (i + 1) % 2)));
            }

            var watch = Stopwatch.StartNew();
            var result = Merge.Join(segments, zero);
            watch.Stop();
        ...
        public void Join_EndpointsStraddlingAGridCellBoundary_StillMeet()
        {
            ...
            for (int step = 0; step <= 20; step++)
            {
                ...
                var segments = new[]
                {
                    new GeoLine(new GeoPoint(x - 4, 0), new GeoPoint(x, 0)),
                    new GeoLine(new GeoPoint(x + 0.4, 0), new GeoPoint(x + 0.4, 5))  // 0.4 away, inside tolerance
                };

                Assert.True(
                    Merge.Join(segments, tolerance).Length == 1,
        ...
        public void Join_KnownChains_ComeBackWholeWhateverTheInputOrder()
        {
            ...
            var result = Merge.Join(shuffled, Tolerance.Global);
        ...
        public void Join_ManySegmentsMeetingAtOnePoint_StaysCheap()
        {
            ...
            var segments = new List<GeoLine>(spokes);
            for (int i = 0; i < spokes; i++)
            {
                double angle = 2.0 * Math.PI * i / spokes;
                segments.Add(new GeoLine(new GeoPoint(0, 0), new GeoPoint(Math.Cos(angle) * 100.0, Math.Sin(angle) * 100.0)));
            }
        ...
        /// </summary>
        private static List<List<GeoPoint>> Staircases(int chains, int segmentsPerChain, out GeoLine[] segments)
        {
            var random = new Random(12345);
            var expected = new List<List<GeoPoint>>();
            var built = new List<GeoLine>();

            for (int c = 0; c < chains; c++)
            {
                // Chains are set far enough apart that none of them can touch another.
                var at = new GeoPoint(c * 1000.0, c * 1000.0);
        ...
        private static GeoLine[] ShuffledStaircases(int chains, int segmentsPerChain, int seed)
        {
            Staircases(chains, segmentsPerChain, out var segments);
            return Shuffle(segments, seed);
        }

        private static GeoLine[] Shuffle(GeoLine[] segments, int seed)
        {
            var copy = (GeoLine[])segments.Clone();
        ...
        private static List<GeoPoint> Vertices(GeoPolyline polyline)
        {
            var vertices = new List<GeoPoint>(polyline.VertexCount);
        ...
        private static IEnumerable<GeoPoint> Ends(GeoPolyline polyline)
        {
            yield return polyline[0];
            yield return polyline[polyline.VertexCount - 1];
        }

        private static string Describe(IEnumerable<GeoPoint> vertices)
        {
        ...
        private static string Describe(GeoPolyline[] polylines)
        {
            var text = new System.Text.StringBuilder();
        ...
        private static void AssertVertices(GeoPolyline polyline, params GeoPoint[] expected)
        {
