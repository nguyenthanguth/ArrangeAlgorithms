...
namespace ArrangeAlgorithms.UnitTest.Core
{
    ...
    public class SplitionEquivalenceTests
    {
        ...
        private static GeoPoint Somewhere(Random random) =>
            new GeoPoint(random.NextDouble() * 30.0 - 10.0, random.NextDouble() * 30.0 - 10.0);

        private static GeoLine RandomLine(Random random) =>
            new GeoLine(Somewhere(random), Somewhere(random));

        private static GeoPolyline RandomPath(Random random)
        {
            var vertices = new GeoPoint[random.Next(2, 5)];
            double x = -10.0;
            for (int i = 0; i < vertices.Length; i++)
            {
                vertices[i] = new GeoPoint(x, random.NextDouble() * 20.0 - 10.0);
                x += 1.0 + random.NextDouble() * 9.0;
            }
            return new GeoPolyline(vertices);
        }

        private static GeoPolygon RandomSquare(Random random)
        {
            double x = random.NextDouble() * 10.0 - 5.0;
            double y = random.NextDouble() * 10.0 - 5.0;
            double size = 2.0 + random.NextDouble() * 8.0;
            return new GeoPolygon(
                new GeoPoint(x, y), new GeoPoint(x + size, y),
                new GeoPoint(x + size, y + size), new GeoPoint(x, y + size));
        }

        private static void SameLines(GeoLine[] a, GeoLine[] b, string what, int c)
        {
        ...
        private static void SamePaths(GeoPolyline[] a, GeoPolyline[] b, string what, int c)
        {
            Assert.True(a.Length == b.Length, $"case {c}: {what} count {a.Length} vs {b.Length}");
            for (int i = 0; i < a.Length; i++)
            {
        ...
        public void Line_CutByOneLine_AnswersTheSameSinglyAndInAnArray()
        {
            ...
            {
                GeoLine subject = RandomLine(random);
                GeoLine cutter = RandomLine(random);

                bool single = subject.TrySplitBy(cutter, out GeoLine first, out GeoLine second, Tol);
                bool array = subject.TrySplitBy(new[] { cutter }, out GeoLine[] pieces, Tol);
        ...
        public void Line_CutByOnePolygon_AnswersTheSameSinglyAndInAnArray()
        {
            ...
            {
                GeoLine subject = RandomLine(random);
                GeoPolygon cutter = RandomSquare(random);

                bool single = subject.TrySplitBy(cutter, out GeoLine[] inA, out GeoLine[] outA, Tol);
                bool array = subject.TrySplitBy(new[] { cutter }, out GeoLine[] inB, out GeoLine[] outB, Tol);
        ...
        public void Line_CutByOnePolylineCutter_AnswersTheSameSinglyAndInAnArray()
        {
            ...
            {
                GeoLine subject = RandomLine(random);
                GeoPolyline cutter = RandomPath(random);

                bool single = subject.TrySplitBy(cutter, out GeoLine[] a, Tol);
                bool array = subject.TrySplitBy(new[] { cutter }, out GeoLine[] b, Tol);
        ...
        public void Polyline_CutByOneLine_AnswersTheSameSinglyAndInAnArray()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                GeoPolyline subject = RandomPath(random);
                GeoLine cutter = RandomLine(random);

                bool single = subject.TrySplitBy(cutter, out GeoPolyline[] a, Tol);
                bool array = subject.TrySplitBy(new[] { cutter }, out GeoPolyline[] b, Tol);
        ...
        public void Polyline_CutByOnePolygon_AnswersTheSameSinglyAndInAnArray()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                ...
                bool single = subject.TrySplitBy(cutter, out GeoPolyline[] inA, out GeoPolyline[] outA, Tol);
                bool array = subject.TrySplitBy(new[] { cutter }, out GeoPolyline[] inB, out GeoPolyline[] outB, Tol);
        ...
        public void CuttingAtAPoint_AnswersTheSameSinglyAndInAnArray()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                ...
                GeoPoint onIt = subject.GetPointAtDistance(subject.Length * (0.05 + random.NextDouble() * 0.9));
                GeoPoint offIt = new GeoPoint(onIt.X, onIt.Y + 3.0);

                foreach (GeoPoint probe in new[] { onIt, offIt })
                {
                    bool single = subject.TrySplitBy(probe, out GeoLine first, out GeoLine second, Tol);
                    bool array = subject.TrySplitBy(new[] { probe }, out GeoLine[] pieces, Tol);
        ...
        public void EveryForm_PreservesTheSubjectLength()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                GeoPolyline subject = RandomPath(random);
                GeoLine cutter = RandomLine(random);
                GeoPolygon polygon = RandomSquare(random);

                subject.TrySplitBy(cutter, out GeoPolyline[] byCutter, Tol);
                subject.TrySplitBy(new[] { cutter }, out GeoPolyline[] byCutterArray, Tol);
                subject.TrySplitBy(polygon, out GeoPolyline[] inside, out GeoPolyline[] outside, Tol);
                subject.TrySplitBy(new[] { polygon }, out GeoPolyline[] within, out GeoPolyline[] beyond, Tol);
        ...
        public void TheSameSegment_SplitsTheSameWhetherItIsALineOrATwoVertexPolyline()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                GeoLine asLine = RandomLine(random);
                var asPath = new GeoPolyline(asLine.StartPoint, asLine.EndPoint);

                GeoPolygon one = RandomSquare(random);
                GeoPolygon two = RandomSquare(random);
                var cutters = new[] { one, two };

                bool lineOk = asLine.TrySplitBy(cutters, out GeoLine[] lineIn, out GeoLine[] lineOut, Tol);
                bool pathOk = asPath.TrySplitBy(cutters, out GeoPolyline[] pathIn, out GeoPolyline[] pathOut, Tol);
                ...
                foreach (GeoPolyline run in pathIn.Concat(pathOut))
                {
