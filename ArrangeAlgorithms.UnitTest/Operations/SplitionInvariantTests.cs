...
namespace ArrangeAlgorithms.UnitTest.Operations
{
    ...
    public class SplitionInvariantTests
    {
        ...
        /// </summary>
        private static GeoPolyline RandomPath(Random random, int vertexCount)
        {
            var vertices = new GeoPoint[vertexCount];
            double x = 0.0;
        ...
        public void Polyline_PiecesCoverTheSubjectExactlyOnce()
        {
            ...
            {
                GeoPolyline path = RandomPath(random, random.Next(2, 9));
                ...
                {
                    GeoPoint end = pieces[i - 1][pieces[i - 1].VertexCount - 1];
                    GeoPoint start = pieces[i][0];
                ...
                GeoPolyline last = pieces[pieces.Length - 1];
        ...
        public void Polyline_SplittingIsSymmetricUnderReversal()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                ...
                GeoPolyline[] forward = Splition.SplitAtDistances(path, cuts, Tol);

                // The same cuts measured from the other end of the same path.
                double[] mirrored = cuts.Select(cut => length - cut).ToArray();
                GeoPolyline[] backward = Splition.SplitAtDistances(path.Reverse(), mirrored, Tol);
                ...
                {
                    GeoPolyline expected = forward[i];
                    GeoPolyline actual = backward[backward.Length - 1 - i].Reverse();
        ...
        public void Polyline_SplittingByAPointMatchesSplittingAtItsDistance()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                ...
                GeoPoint point = path.GetPointAtDistance(distance);

                bool byDistance = Splition.TrySplitAtDistance(path, distance, out GeoPolyline a1, out GeoPolyline a2, Tol);
                bool byPoint = Splition.TrySplitBy(path, point, out GeoPolyline b1, out GeoPolyline b2, Tol);
        ...
        public void Polyline_SplittingByALineMatchesSplittingAtTheCrossings()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                ...
                var cutter = new GeoLine(new GeoPoint(-50, y), new GeoPoint(right + 50, y));

                if (Splition.TrySplitBy(path, cutter, out GeoPolyline[] pieces, Tol))
                {
                    Assert.Equal(path.Length, pieces.Sum(p => p.Length), 6);
                    ...
                    for (int i = 1; i < pieces.Length; i++)
                    {
                        Assert.True(Containment.IsPointOn(cutter, pieces[i][0], Tol),
        ...
        public void Line_PiecesCoverTheSegmentExactlyOnce()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                var line = new GeoLine(
                    new GeoPoint(random.NextDouble() * 20.0 - 10.0, random.NextDouble() * 20.0 - 10.0),
                    new GeoPoint(random.NextDouble() * 20.0 + 10.0, random.NextDouble() * 20.0 + 10.0));

                double[] cuts = RandomCuts(random, line.Length, random.Next(0, 6));
                GeoLine[] pieces = Splition.SplitAtDistances(line, cuts, Tol);
        ...
        public void NoValidInputThrows()
        {
            ...
            for (int c = 0; c < Cases; c++)
            {
                ...
                Splition.TrySplitAtDistance(path, double.NaN, out _, out _, Tol);
                Splition.TrySplitBy(path, new GeoPoint(double.NaN, double.NaN), out GeoPolyline _, out GeoPolyline _, Tol);
                Splition.TrySplitBy(path, new GeoLine(new GeoPoint(0, 0), new GeoPoint(0, 0)), out _, Tol);
            }
        }
    }
}
