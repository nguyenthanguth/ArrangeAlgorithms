...
namespace ArrangeAlgorithms.UnitTest
{
    ...
    public class ArrangeExtendedTests
    {
        ...
        private static Arrange LabelOn(GeoLine leader, double width = 20.0, double height = 10.0)
        {
            return new Arrange
            {
                GeoLine = leader,
        ...
        public void Arrange_Run_HighPerpendicularLevels_CanPlaceManyLabels(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(0.0, 0.0, 80.0, 0.0);
        ...
        public void Arrange_Run_Leader30Degrees_PlacesWithoutOverlap(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(0, 0, len * Math.Cos(angle), len * Math.Sin(angle));
            var a = LabelOn(leader);
            var b = LabelOn(leader);
        ...
        public void Arrange_Run_LeaderPointingDown_PlacesLabelsCorrectly(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(50.0, 100.0, 50.0, 0.0);
            var a = LabelOn(leader);
            var b = LabelOn(leader);
        ...
        public void Arrange_Run_NegativeCoordinateLeader_PlacesCorrectly(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(-200.0, -100.0, -100.0, -100.0);
            var a = LabelOn(leader);
            var b = LabelOn(leader);
        ...
        public void Arrange_Run_NestedPolygonObstacles_AvoidsAllLayers(ArrangeAlgorithmType algorithm)
        {
            ...
            var outerBox = new GeoPolygon(
                new GeoPoint(-50, 2), new GeoPoint(90, 2), new GeoPoint(90, 25), new GeoPoint(-50, 25));

            var label = LabelOn(leader);
            label.BlockPolygons = new List<GeoPolygon> { innerBox, outerBox };
            ...
            Assert.False(moved.IntersectsWith(innerBox));
            Assert.False(moved.IntersectsWith(outerBox));
            Assert.True(label.Placed);
        }
        ...
        public void Arrange_Run_ParallelBlockLines_ForcesLabelsToLowerLevel(ArrangeAlgorithmType algorithm)
        {
            ...
            {
                new GeoLine(-100, 8.0, 100, 8.0),
                new GeoLine(-100, 12.0, 100, 12.0)
            };
        ...
        public void Arrange_Run_PolygonObstacleCoveringEntireTopSide_ForcesBottom(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(0.0, 0.0, 40.0, 0.0);
            var topBlock = new GeoPolygon(
                new GeoPoint(-200, 0), new GeoPoint(200, 0), new GeoPoint(200, 200), new GeoPoint(-200, 200));

            var label = LabelOn(leader);
            label.BlockPolygons = new List<GeoPolygon> { topBlock };
            ...
            var moved = MovedBox(label, label.TranslationVector);
            Assert.True(moved.Center.Y < 0.0);
            Assert.False(moved.IntersectsWith(topBlock));
            Assert.True(label.Placed);
        }
        ...
        public void Arrange_Run_CrossShapedObstacles_FindsCornerPosition(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(0.0, 0.0, 80.0, 0.0);
            var horizontalBar = new GeoPolygon(
                new GeoPoint(-200, -3), new GeoPoint(200, -3), new GeoPoint(200, 3), new GeoPoint(-200, 3));
            var verticalBar = new GeoPolygon(
                new GeoPoint(37, -200), new GeoPoint(43, -200), new GeoPoint(43, 200), new GeoPoint(37, 200));

            var label = LabelOn(leader);
            label.BlockPolygons = new List<GeoPolygon> { horizontalBar, verticalBar };
            ...
            var moved = MovedBox(label, label.TranslationVector);
            Assert.False(moved.IntersectsWith(horizontalBar));
            Assert.False(moved.IntersectsWith(verticalBar));
            Assert.True(label.Placed);
        }
        ...
        {
            var leader = new GeoLine(0.0, 0.0, 30.0, 0.0);
            var a = LabelOn(leader);
            var b = LabelOn(leader);
        ...
        public void Arrange_Run_LabelsOnPerpendicularLeaders_NoOverlap(ArrangeAlgorithmType algorithm)
        {
            ...
            var horizontal = new GeoLine(0.0, 0.0, 60.0, 0.0);
            var vertical = new GeoLine(0.0, 0.0, 0.0, 60.0);
        ...
        public void Arrange_Run_RelaxationPass_MultipleFailedLabelsGetBestEffortPositions(ArrangeAlgorithmType algorithm)
        {
            ...
            failedA.BlockLines = new List<GeoLine>
            {
                new GeoLine(-100, 10.0, 100, 10.0),
                new GeoLine(-100, -10.0, 100, -10.0)
            };
            failedB.BlockLines = new List<GeoLine>
            {
                new GeoLine(100, 10.0, 300, 10.0),
                new GeoLine(100, -10.0, 300, -10.0)
            };
            ...
            Assert.NotEqual(GeoVector.Zero, failedA.TranslationVector);
            Assert.NotEqual(GeoVector.Zero, failedB.TranslationVector);
        }
        ...
        public void Arrange_Run_RelaxationPass_FrozenLabelsBlockPolygonsDoNotAffectOwnPosition(ArrangeAlgorithmType algorithm)
        {
            ...
            Assert.NotEqual(GeoVector.Zero, successLabel.TranslationVector);
        }
        ...
        public void Arrange_Run_LabelsWithDifferentSizes_PlacesCorrectly(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader1 = new GeoLine(0.0, 0.0, 40.0, 0.0);
            var leader2 = new GeoLine(100.0, 0.0, 200.0, 0.0);
        ...
        public void Arrange_Run_PlaceFromInsideOutDisabled_StillSolvesCorrectly(ArrangeAlgorithmType algorithm)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                var leader = new GeoLine(i * 60, 0, i * 60 + 40, 0);
                labels.Add(LabelOn(leader));
            }
        ...
        public void Arrange_Run_PlaceMostConstrainedFirstDisabled_StillSolvesCorrectly(ArrangeAlgorithmType algorithm)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                var leader = new GeoLine(i * 80, 0, i * 80 + 40, 0);
                labels.Add(LabelOn(leader));
            }
        ...
        public void Arrange_Run_BlockLinesAndBlockPolygonsCombined_AvoidsAll(ArrangeAlgorithmType algorithm)
        {
            ...
            label.BlockLines = new List<GeoLine>
            {
                new GeoLine(-100, 10.0, 200, 10.0) // Blocks upper level 1
            };
            label.BlockPolygons = new List<GeoPolygon>
            {
                new GeoPolygon(
                    new GeoPoint(-100, -20), new GeoPoint(200, -20),
                    new GeoPoint(200, -5), new GeoPoint(-100, -5)) // Blocks lower level 1
            };
            ...
            var moved = MovedBox(label, label.TranslationVector);
            Assert.False(moved.IntersectsWith(label.BlockLines[0]));
            Assert.False(moved.IntersectsWith(label.BlockPolygons[0]));
            Assert.True(label.Placed);
        }
        ...
        public void Arrange_Run_MultipleBlockLinesFromDifferentAngles_AvoidsAll(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(0.0, 0.0, 60.0, 0.0);
            var label = LabelOn(leader);
            label.BlockLines = new List<GeoLine>
            {
                new GeoLine(0, 5, 30, 30),   // Diagonal line from left
                new GeoLine(60, 5, 30, 30)    // Diagonal line from right
            };
            ...
            {
                Assert.False(moved.IntersectsWith(bl));
            }
            Assert.True(label.Placed);
        }
        ...
        public void Arrange_Run_TenLabelsOnSingleLongLeader_NoPairwiseOverlap(ArrangeAlgorithmType algorithm)
        {
            ...
            for (int i = 0; i < placed.Count; i++)
            {
                for (int j = i + 1; j < placed.Count; j++)
                {
                    ...
                    Assert.False(boxI.IntersectsWith(boxJ));
                }
            }
        }
        ...
        public void Arrange_Run_LabelAtOriginWithFarObstacle_NotAffectedByDistantObstacle(ArrangeAlgorithmType algorithm)
        {
            ...
            var leader = new GeoLine(0.0, 0.0, 40.0, 0.0);
            var label = LabelOn(leader);
            var farObstacle = new GeoPolygon(
                new GeoPoint(10000, 10000), new GeoPoint(10100, 10000),
                new GeoPoint(10100, 10100), new GeoPoint(10000, 10100));
            label.BlockPolygons = new List<GeoPolygon> { farObstacle };
