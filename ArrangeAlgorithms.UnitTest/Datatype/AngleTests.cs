...
namespace ArrangeAlgorithms.UnitTest.Datatype
{
    public class AngleTests
    {
        ...
        #region Integration with the geometry types
        ...
        public void Angle_FeedsTheGeometryApisThatTakeRadians()
        {
            ...
            var rotated = GeoVector.XAxis.RotateBy(quarterTurn.Radians);
            Assert.True(rotated.IsEqualTo(GeoVector.YAxis));

            var rect = new GeoRectangle(new GeoPoint(0, 0), 4, 2, quarterTurn.Radians);
            Assert.True(rect.IsRotated);

            // A full turn is not a rotation, which is what GeoRectangle.IsRotated relies on.
            var fullTurn = new GeoRectangle(new GeoPoint(0, 0), 4, 2, Angle.FullTurn.Radians);
