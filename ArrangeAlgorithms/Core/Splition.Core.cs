...
namespace ArrangeAlgorithms.Core
{
    ...
    public static partial class Splition
    {
        ...
        /// which in turn keeps the results clear of the degenerate cases that make the GeoPolyline
        ...
        /// </remarks>
        internal static bool TryGetCutDistance(GeoLine source, GeoPoint point, Tolerance tolerance, out double distance)
        {
            distance = 0.0;
            if (!Containment.IsPointOn(source, point, tolerance))
            {
                return false;
            }

            distance = Parametrization.GetDistanceAtPoint(source, point);
            return true;
        }
        ...
        /// do not lie on the subject.
        /// </summary>
        internal static bool TryGetCutDistance(GeoPolyline source, GeoPoint point, Tolerance tolerance, out double distance)
        {
            distance = 0.0;
        ...
        /// </summary>
        internal static double[] ToCutDistances(GeoLine subject, GeoPoint[] crossings, Tolerance tolerance)
        {
            ...
            {
                distances[i] = Parametrization.GetDistanceAtPoint(subject, crossings[i]);
            }
        ...
        /// </summary>
        internal static double[] ToCutDistances(GeoPolyline subject, GeoPoint[] crossings, Tolerance tolerance)
        {
            if (crossings.Length == 0)
            {
                return NoCuts;
        ...
        /// </summary>
        internal static GeoLine[] SplitLineAt(GeoLine source, double[] cuts)
        {
            if (cuts.Length == 0)
            {
                return new[] { source };
            ...
            var pieces = new GeoLine[cuts.Length + 1];
            GeoPoint previous = source.StartPoint;

            for (int i = 0; i < cuts.Length; i++)
            {
                GeoPoint cutPoint = Parametrization.GetPointAtDistance(source, cuts[i]);
                pieces[i] = new GeoLine(previous, cutPoint);
                previous = cutPoint;
            }

            pieces[cuts.Length] = new GeoLine(previous, source.EndPoint);
            return pieces;
        }
        ...
        /// </remarks>
        internal static GeoPolyline[] SplitPolylineAt(GeoPolyline source, double[] cuts, Tolerance tolerance)
        {
            if (source == null) throw new ArgumentNullException(nameof(source));
            ...
            BuildStations(source, cuts, tolerance, out GeoPoint[] stations, out int[] boundaries);
            ...
            var pieces = new GeoPolyline[boundaries.Length + 1];
        ...
        private static void BuildStations(
            GeoPolyline source, double[] cuts, Tolerance tolerance,
            out GeoPoint[] stations, out int[] boundaries)
        {
            int edgeCount = source.EdgeCount;
            double margin = tolerance.EqualPoint;

            var points = new List<GeoPoint>(edgeCount + 1 + cuts.Length) { source[0] };
            ...
            {
                GeoLine edge = source.GetEdgeAt(i);
                ...
                while (nextCut < cuts.Length && cuts[nextCut] < edgeEnd - margin)
                {
                    ...
                    GeoPoint cutPoint = edgeLength > 0.0
        ...
        /// </summary>
        private static GeoPolyline Slice(GeoPoint[] stations, int from, int to)
        {
            int count = to - from + 1;
            var vertices = new GeoPoint[count];
            Array.Copy(stations, from, vertices, 0, count);
            return new GeoPolyline(vertices, count);
        }
    }
}
