...
namespace ArrangeAlgorithms.Core
{
    ...
    public static class Merge
    {
        ...
        /// </para>
        /// </remarks>
        public static GeoLine[] ConsecutiveLines(IEnumerable<GeoLine> segments, Tolerance tolerance)
        {
        ...
        /// </remarks>
        public static GeoPolyline Polylines(GeoPolyline first, GeoPolyline second, Tolerance tolerance)
        {
        ...
        /// </remarks>
        public static GeoPolyline[] ConsecutivePolylines(IEnumerable<GeoPolyline> polylines, Tolerance tolerance)
        {
            ...
            using (var enumerator = polylines.GetEnumerator())
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    var merged = Merge.Polylines(current, next, tolerance);
                    if (merged != null)
                    {
        ...
        /// </para>
        /// </remarks>
        public static GeoPolyline[] Join(IEnumerable<GeoLine> lines, Tolerance tolerance)
        {
            ...
            var joined = new List<GeoPolyline>();

            var ahead = new List<GeoPoint>();
            var behind = new List<GeoPoint>();
            ...
            for (int seed = 0; seed < segments.Count; seed++)
            {
                ...
                var run = new List<GeoPoint>(behind.Count + ahead.Count - 1);
        ...
        /// and yield a polyline with two identical vertices, which the public GeoPolyline constructor
        ...
        /// </para>
        /// </remarks>
        public static GeoPolyline[] JoinBackup(IEnumerable<GeoLine> lines, Tolerance tolerance)
        {
            if (lines == null) throw new ArgumentNullException(nameof(lines));

            var resultPolylines = new List<List<GeoPoint>>();
            foreach (var line in lines)
            {
                ...
                resultPolylines.Add(new List<GeoPoint> { line.StartPoint, line.EndPoint });
            }
            ...
            do
            {
                ...
                for (int i = 0; i < resultPolylines.Count; i++)
                {
                    for (int j = i + 1; j < resultPolylines.Count; j++)
                    {
                        ...
                        GeoPoint start1 = poly1[0];
                        GeoPoint end1 = poly1[poly1.Count - 1];
                        GeoPoint start2 = poly2[0];
                        GeoPoint end2 = poly2[poly2.Count - 1];
                        ...
                        {
                            var rev2 = new List<GeoPoint>(poly2);
                            rev2.Reverse();
            ...
            var finalPolylines = new List<GeoPolyline>();
        ...
        /// </summary>
        private static void Extend(List<GeoLine> segments, EndpointGrid grid, bool[] consumed, List<GeoPoint> run, Tolerance tolerance)
        {
            while (true)
            {
                GeoPoint tip = run[run.Count - 1];
        ...
        /// </remarks>
        private static GeoPolyline Simplify(List<GeoPoint> run, Tolerance tolerance)
        {
            var simplified = new List<GeoPoint> { run[0] };
            for (int i = 1; i < run.Count - 1; i++)
            {
                GeoPoint prev = simplified[simplified.Count - 1];
                GeoPoint curr = run[i];
                GeoPoint next = run[i + 1];

                if (!Containment.IsPointOn(new GeoLine(prev, next), curr, tolerance))
                {
                    simplified.Add(curr);
                }
            }
            ...
            return new GeoPolyline(simplified.ToArray(), simplified.Count);
        }
        ...
        private sealed class EndpointGrid
        {
            ...
            private readonly List<GeoLine> _segments;
            private readonly Tolerance _tolerance;
            private readonly double _cellSize;

            internal EndpointGrid(List<GeoLine> segments, Tolerance tolerance)
            {
                _segments = segments;
            ...
            /// </summary>
            internal int TakeSegmentAt(GeoPoint point, bool[] consumed)
            {
            ...
            private void Add(GeoPoint point, int segment)
            {
            ...
            private Cell CellOf(GeoPoint point)
            {
            ...
            /// </para>
            /// </remarks>
            private int TakeFrom(List<int> bucket, GeoPoint point, bool[] consumed)
            {
